...
### F-17 — Dispatcher-mediated routing, not node-to-node hand-off *(P0 — corrects §6.2)*

**This is the most consequential Phase −1 finding and changes the Phase 4 graph design.** Its original justification was checked against Anthropic's primary docs and corrected below — the architectural recommendation stands, the reason it stands does not.

**Correction of the original claim:** B3's `references/orchestration-patterns.md` was quoted as citing a hard platform wall — *"subagents cannot spawn other subagents"*. Verified directly against `code.claude.com/docs/en/sub-agents`: this is **false as a platform limit**. Claude Code subagents *can* nest, by default up to 3 layers below the main conversation (configurable via `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH`, settable to `1` to disable nesting or higher to extend it). `Architect → TechLead → Engineering` is technically buildable as a direct call chain. B3's "personas don't invoke personas" reads as a self-imposed team convention presented with platform-constraint framing, not a fact about the runtime — the claim is not reused here.

**The recommendation is unchanged, for four reasons that hold independent of platform capability:**

1. **Context fidelity.** Per the verified docs, a non-fork subagent does *not* inherit "main conversation history, parent session's auto memory, previously invoked skills' context" — it starts fresh, seeded only by whatever the calling agent writes into its delegation message. A 3-level call chain (`TechLead → Engineering → Reviewer`) means Reviewer's picture of the original goal is a paraphrase of a paraphrase. This is the same failure mode already named in §6.1 (*"nodes re-parse prose written by another node"*) — nesting doesn't fix it, it compounds it. A single dispatcher reading/writing one `state.json` gives every node the same complete, un-degraded state slice regardless of how many nodes ran before it.
2. **Matches the reference diagram's own separation.** The user-supplied diagram (B5) draws **Harness** as its own box — *"keeps it alive"* — distinct from the Nodes that do work. That box is the scheduler, not another node. Standard graph/workflow runtimes (LangGraph, Temporal, Airflow, AWS Step Functions) are built exactly this way: a central runtime evaluates state and invokes node functions; nodes are not aware of each other and never call each other directly, regardless of whether the underlying platform would technically permit it. Dispatcher-mediation isn't a workaround for a limitation — it's the standard shape of the thing being asked for.
3. **Auditability.** If the repair-loop condition (*"Reviewer has open findings → re-invoke Engineering"*) lives inside Reviewer's own prompt three levels deep, the graph's actual control flow is scattered across agent definitions and invisible from outside. With one dispatcher, every edge predicate lives in one place (`.agents/graph.md`), inspectable and editable without touching any agent's prompt.
4. **Cross-harness portability (§7's own design rule).** Depth limits and tool-inheritance-at-depth-limit are Claude-Code-specific mechanics with no equivalent in agy/OpenCode/Codex. A flat dispatcher loop (*invoke agent A, read state, invoke agent B, read state, …*) is trivially expressible in every harness without reinventing per-harness nesting semantics.

The graph design proposed in §6.2 below — `Architect → TechLead → {UI_UX ∥ Engineering ∥ Media} → Reviewer → Shipping` — is corrected to make explicit that it was never meant as node-to-node hand-off: the **dispatcher is the main session or a slash command**; nodes are leaves that read state, do work, write state, and return — the *routing decision* (which edge fires next) is evaluated by the dispatcher after each return, not delegated into the graph itself. No node, edge, or capability from the original design is lost — only the "who calls whom" framing is corrected.

**Two refinements from checking the primary docs directly (`sub-agents`, `agent-teams`), both narrowing rather than reversing the recommendation:**

- **Hooks propagate regardless of nesting** — *"a `PreToolUse` hook in `settings.json` also runs before every tool a subagent uses"* — so the GO-gate hook (Phase 1) covers the whole tree automatically; hook coverage was never a reason to prefer dispatcher-mediation and isn't used as one above.
- **Bounded fan-out inside a single node is explicitly sanctioned and different from top-level chaining** — the docs' own example is *"a reviewer subagent that dispatches a verifier per finding, so the intermediate output never reaches your main conversation."* This is fine, and worth using: the Reviewer node in §6.2 can internally fan out to one verifier per finding without violating F-17, because from the dispatcher's perspective Reviewer is still a single leaf call that returns one summary. F-17 constrains the *top-level pipeline stages* from calling each other; it says nothing against a node organizing its own internal work.
- **Agent Teams (`CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS`) was considered and rejected for `/startcycle` specifically.** It offers a genuinely richer coordination primitive — a shared, dependency-aware task list, direct teammate-to-teammate messaging, and `TeammateIdle`/`TaskCreated`/`TaskCompleted` hooks for quality gates — that would fit a graph model closely. It is disqualified for this repo's core pipeline by one stated limitation: *"Spawning teammates also requires an interactive session. In non-interactive mode with the `-p` flag ... Claude doesn't spawn teammates."* `/startcycle` is documented as "zero-prompting" and needs to run unattended/headless; Agent Teams cannot. It remains a good candidate for `/bdbrainstorm` (its "competing hypotheses" pattern suits open-ended ideation) but not for the deterministic build pipeline — noted here so a future contributor doesn't rediscover and reverse this choice without the reasoning.
...
**Correction (F-17):** the version of this section as originally written implied nodes hand off directly to each other (`Architect → TechLead → {...} → Reviewer → Shipping` read as a call chain). Claude Code actually permits nested subagent calls up to 3 levels deep by default — this is not platform-forbidden — but F-17 recommends against it anyway: nested subagents don't inherit the caller's full context, so a 3-level chain compounds exactly the "state gets re-parsed as prose" problem already flagged in §6.1, and depth-limited nesting is Claude-Code-specific machinery with no equivalent in the other harnesses this repo targets. The graph below keeps the same nodes and edges but makes explicit that **routing is evaluated by a single dispatcher** (the main session, or a slash command like `/startcycle` acting on the user's behalf) after each node returns — the arrows are dispatcher decisions, not node-to-node calls.
...
- *Advisory:* the graph contract in `.agents/graph.md`, read by the dispatcher (main session or `/startcycle` command logic) — this is also where the F-17 reasoning is written down explicitly, so a future contributor doesn't "optimize" the pipeline into direct node-to-node hand-offs and reintroduce the context-fidelity problem F-17 exists to avoid.
