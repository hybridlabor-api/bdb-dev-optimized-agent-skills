...
### 4. UI/UX Pro Max & Anti-Slop Standards
- Every user interface decision, wireframe, or component generated during this process MUST adhere to the `godmode-ui-ux` and `ui-ux-pro-max` skill guidelines.
...
### 5. Autonomous Pipeline Handoff (`/startcycle`)
- Once the brainstorming, grilling, and spec alignment are completed, the master orchestrator MUST present the summary and trigger or suggest running `/startcycle`.
- `/startcycle` ingests the brainstorming plan and executes the parallel 4-phase implementation pipeline (`Planner_Orchestrator` ➔ `Godmode_UI_UX` + `Godmode_Engineering` + `Godmode_Media_EventTech` ➔ `Godmode_Shipping` ➔ `.openwiki` / `memB`).
...
4. **Always close the loop:** Hand off final specs to `/startcycle` for execution.
