...
def build_ai_vault(memory: Any):
    ...
    # Write _CLAUDE.md
    with open(os.path.join(vault_dir, "_CLAUDE.md"), "w", encoding="utf-8") as f:
        f.write("# memB Vault Operating Manual\n\nThis vault is auto-generated by memB for native AI agent access.\nAgents can use grep, list_dir, and view_file to query memory directly without the MCP.\nIt follows a strict Top-Down Radial Tree topology (God Mode -> Projects -> Categories -> Memories).\n")
    
    all_mem = memory.get_all(user_id="bdb_developer", limit=10000)
