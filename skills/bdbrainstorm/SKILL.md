---
name: bdbrainstorm
description: Combines multi-agent brainstorming, the /grill-me slash command, and the 3 Core Godmodes (engineering-godmode, ui-ux-godmode, shipping-godmode) to force a comprehensive, multi-agent ideation and technical design workflow.
---

# BDBrainstorm: The Ultimate Multi-Agent Design & Ideation Workflow

When this skill is invoked or requested, you MUST orchestrate a comprehensive ideation and development workflow using a combination of multi-agent brainstorming, interactive user grilling, and strict adherence to the 3 Core Godmodes (`engineering-godmode`, `ui-ux-godmode`, `shipping-godmode`). 
...
- Once the conceptual plan is solid, you must strictly enforce `engineering-godmode` principles.
...
- Every user interface decision, wireframe, or component generated during this process MUST adhere to the `ui-ux-godmode` guidelines.
...
- Before the brainstorm concludes and the code is merged, verify that the plan satisfies the `shipping-godmode` rules (Spec-Driven Development, feature flags, rollback strategies).
...
3. **Never accept ugly UI:** Always apply `ui-ux-godmode` rules.
4. **Never skip architecture & shipping checks:** Always enforce `engineering-godmode` and `shipping-godmode` constraints.
